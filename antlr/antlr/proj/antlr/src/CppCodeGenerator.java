...
public class CppCodeGenerator extends CodeGenerator {
	...
	public void genBody(LexerGrammar g) throws IOException
	{
		...
		//
		println(grammar.getClassName() + "::" + grammar.getClassName() + "(antlr::pool_parser pool," + namespaceStd + "istream& in)");
		tabs++;
		// if debugging, wrap the input buffer in a debugger
		if (grammar.debuggingOutput)
			println(": " + sup + "(pool,new(*pool,get_ptr<antlr::DebuggingInputBuffer>()) "+namespaceAntlr+"DebuggingInputBuffer(new(*pool,get_ptr<antlr::CharBuffer>()) "+namespaceAntlr+"CharBuffer(in)),"+g.caseSensitive+")");
		else
			println(": " + sup + "(pool,new(*pool,get_ptr<antlr::CharBuffer>()) "+namespaceAntlr+"CharBuffer(in),"+g.caseSensitive+")");
		tabs--;
		println("{");
		tabs++;
...
//		println("setCaseSensitive("+g.caseSensitive+");");
		...
		println(grammar.getClassName() + "::" + grammar.getClassName() + "(antlr::pool_parser pool,"+namespaceAntlr+"InputBuffer& ib)");
		tabs++;
		// if debugging, wrap the input buffer in a debugger
		if (grammar.debuggingOutput)
			println(": " + sup + "(pool,new(pool) "+namespaceAntlr+"DebuggingInputBuffer(ib),"+g.caseSensitive+")");
		else
			println(": " + sup + "(pool,ib,"+g.caseSensitive+")");
		tabs--;
		println("{");
		tabs++;
...
//		println("setCaseSensitive("+g.caseSensitive+");");
		...
		println(grammar.getClassName() + "::" + grammar.getClassName() + "(antlr::pool_parser pool,const "+namespaceAntlr+"LexerSharedInputState& state)");
		tabs++;
		println(": " + sup + "(pool,state,"+g.caseSensitive+")");
		tabs--;
		println("{");
		tabs++;
...
//		println("}");
		...
		println("(antlr::pool_parser pool,"+namespaceAntlr+"TokenBuffer& tokenBuf, int k)");
		println(": " + sup + "(pool,tokenBuf,k)");
		println("{");
//		tabs++;
//		println("_initialize();");
//		tabs--;
		...
		print(grammar.getClassName() + "::" + grammar.getClassName());
		println("(antlr::pool_parser pool,"+namespaceAntlr+"TokenBuffer& tokenBuf)");
		println(": " + sup + "(pool,tokenBuf," + grammar.maxk + ")");
		println("{");
//		tabs++;
//		println("_initialize();");
//		tabs--;
		...
		print(grammar.getClassName() + "::" + grammar.getClassName());
		println("(antlr::pool_parser pool,"+namespaceAntlr+"TokenStream& lexer, int k)");
		println(": " + sup + "(pool,lexer,k)");
		println("{");
//		tabs++;
//		println("_initialize();");
//		tabs--;
		...
		print(grammar.getClassName() + "::" + grammar.getClassName());
		println("(antlr::pool_parser pool,"+namespaceAntlr+"TokenStream& lexer)");
		println(": " + sup + "(pool,lexer," + grammar.maxk + ")");
		println("{");
//		tabs++;
//		println("_initialize();");
//		tabs--;
		...
		print(grammar.getClassName() + "::" + grammar.getClassName());
		println("(antlr::pool_parser pool,const "+namespaceAntlr+"ParserSharedInputState& state)");
		println(": " + sup + "(pool,state," + grammar.maxk + ")");
		println("{");
//		tabs++;
//		println("_initialize();");
//		tabs--;
...
//					println(labeledElementASTType+" " + astName + " = "+labeledElementASTInit+";");
	...
	public void genInclude(LexerGrammar g) throws IOException
	{
		...
		println(grammar.getClassName() + "(antlr::pool_parser pool," + namespaceStd + "istream& in);");

		// Generate the constructor from InputBuffer
		println(grammar.getClassName() + "(antlr::pool_parser pool,"+namespaceAntlr+"InputBuffer& ib);");

		println(grammar.getClassName() + "(antlr::pool_parser pool, const "+namespaceAntlr+"LexerSharedInputState& state);");
		if( noConstructors )
		{
			tabs = 0;
			println("// constructor creation turned of with 'noConstructor' option");
...
//		println("void _initialize( void );");
		...
		println("protected:");
		tabs=1;
		println(grammar.getClassName() + "(antlr::pool_parser pool, "+namespaceAntlr+"TokenBuffer& tokenBuf, int k);");
		tabs=0;
		println("public:");
		tabs=1;
		println(grammar.getClassName() + "(antlr::pool_parser pool, "+namespaceAntlr+"TokenBuffer& tokenBuf);");

		// Generate parser class constructor from TokenStream
		tabs=0;
		println("protected:");
		tabs=1;
		println(grammar.getClassName()+"(antlr::pool_parser pool, "+namespaceAntlr+"TokenStream& lexer, int k);");
		tabs=0;
		println("public:");
		tabs=1;
		println(grammar.getClassName()+"(antlr::pool_parser pool,"+namespaceAntlr+"TokenStream& lexer);");

		println(grammar.getClassName()+"(antlr::pool_parser pool, const "+namespaceAntlr+"ParserSharedInputState& state);");
		if( noConstructors )
		{
			tabs = 0;
			println("// constructor creation turned of with 'noConstructor' option");
	...
	public void genNextToken() {
		...
		if (!hasPublicRules) {
			println("");
			println(namespaceAntlr+"RefToken "+grammar.getClassName()+"::nextToken() { return "+namespaceAntlr+"RefToken(pool,new "+namespaceAntlr+"CommonToken("+namespaceAntlr+"Token::EOF_TYPE, \"\")); }");
			println("");
			return;
		}
...
//		}
	...
	protected void genSynPred(SynPredBlock blk, String lookaheadExpr) {
		...
		tabs++;
                println("(void) pe; /* suppress warnings about unreferenced var 'pe' */");
