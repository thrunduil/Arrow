...
public:
	...
	static const char* const DEFAULT_PROGRAM_NAME;
    ...
    typedef std::map<std::string,operation_list> program_map;
	...
	TokenStreamRewriteEngine(pool_parser pool,TokenStream& upstream);

	TokenStreamRewriteEngine(pool_parser pool,TokenStream& upstream, size_t initialSize);
	 ...
	 */
	void rollback(const std::string& programName,
	...
	void insertAfter( const std::string& programName,
							RefTokenWithIndex t,
							const std::string& text )
	{
	...
	void insertBefore( const std::string& programName,
							 RefTokenWithIndex t,
							 const std::string& text )
	{
	...
	void insertBefore( const std::string& programName,
							 ...
							 const std::string& text )
	{
		addToSortedRewriteList(programName, new(*get_pool(),get_ptr<InsertBeforeOp>()) InsertBeforeOp(index,text));
	}
	...
	void replace(const std::string& programName,
					 size_t from, size_t to,
					 const std::string& text )
	{
		addToSortedRewriteList(programName,new(*get_pool(),get_ptr<ReplaceOp>()) ReplaceOp(from, to, text));
	}
	...
	void remove( const std::string& programName,
					 size_t from, size_t to)
	{
...
protected:
	...
	void addToSortedRewriteList( const std::string& programName,
										  RewriteOperation* op );
