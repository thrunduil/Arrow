...
private:
	pool_parser pool;
public:
	TreeParser(pool_parser pool)
	: astFactory(0)
	,pool(pool)
	, inputState(new(*pool,get_ptr<TreeParserInputState>()) TreeParserInputState())
	, traceDepth(0)
	, ASTNULL(new(*pool,get_ptr<ASTNULLType>()) ASTNULLType)
	{
	}
	TreeParser(pool_parser pool,const TreeParserSharedInputState& state)
	: astFactory(0)
	,pool(pool)
	, inputState(state)
	, traceDepth(0)
