...
public:
	LLkParser(pool_parser pool,const ParserSharedInputState& lexer, int k_);

	LLkParser(pool_parser pool,TokenBuffer& tokenBuf, int k_);

	LLkParser(pool_parser pool,TokenStream& lexer, int k_);
	...
	{
		int val = inputState->getInput().LA(i);
		return val;
	}
