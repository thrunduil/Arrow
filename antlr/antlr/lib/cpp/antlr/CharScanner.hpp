 ...
 */
class ANTLR_API CharScannerLiteralsLess : public ANTLR_USE_NAMESPACE(std)binary_function<std::string,std::string,bool> {
private:
...
protected:
    typedef RefToken (*factory_type)(pool_parser pool);
public:
    CharScanner(pool_parser pool,InputBuffer& cb, bool case_sensitive );
    CharScanner(pool_parser pool,InputBuffer* cb, bool case_sensitive );
    CharScanner(pool_parser pool,const LexerSharedInputState& state, bool case_sensitive );
    ...
    virtual void match( const wchar_t* s )
    {
        while( *s != '\0' )
        {
            // the & 0xFF is here to prevent sign extension lateron
            int la_1 = LA(1), c = (*s++ & 0xFFFF);
    ...
    {
        std::map<std::string,int,CharScannerLiteralsLess>::const_iterator i = literals.find(txt);
        if (i != literals.end())
            ttype = (*i).second;
        return ttype;
    }
...
protected:
    ...
    {
        RefToken tok = tokenFactory(get_pool());
        tok->setType(t);
...
inline bool CharScannerLiteralsLess::operator() (const std::string& x,const std::string& y) const
{
    if (scanner->getCaseSensitiveLiterals())
        return ANTLR_USE_NAMESPACE(std)less<std::string>()(x,y);
    else
    {
#ifdef NO_STRCASECMP
