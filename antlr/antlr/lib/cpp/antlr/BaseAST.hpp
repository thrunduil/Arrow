...
public:
   ...
   /// Add a node to the end of the child list for this node
	...
	virtual RefAST getFirstChild() 
	{
		return RefAST(down);
	}
	/// Get  the next sibling in line after this one
	virtual RefAST getNextSibling() 
	{
		return RefAST(right);
	}
	...
	{
		down.set(static_cast<BaseAST*>(static_cast<AST*>(nullAST)));
	}

	/// Set the first child of a node.
	virtual void setFirstChild(RefAST c)
	{
		down.set(static_cast<BaseAST*>(static_cast<AST*>(c)) );
	}

	/// Set the next sibling after this one.
	virtual void setNextSibling(RefAST n)
	{
		right.set(static_cast<BaseAST*>(static_cast<AST*>(n)) );
	}
    ...
    virtual std::string toStringList() ;
    virtual std::string toStringTree() ;
protected:
	RefBaseAST down;
	RefBaseAST right;
private:
	void doWorkForFindAll(std::vector<RefAST>& v,
								 RefAST target,
								 bool partialMatch);
};
