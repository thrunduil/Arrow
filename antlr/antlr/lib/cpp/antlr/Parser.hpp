...
protected:
	Parser(pool_parser pool,TokenBuffer& input)
		: pool(pool),inputState(new(*pool,get_ptr<ParserInputState>()) ParserInputState(input))
		, astFactory(0), traceDepth(0)
	{
	}
	Parser(pool_parser pool,TokenBuffer* input)
	: pool(pool),inputState(new(*pool,get_ptr<ParserInputState>()) ParserInputState(input))
	...
	Parser(pool_parser pool,const ParserSharedInputState& state)
	: pool(pool),inputState(state), astFactory(0), traceDepth(0)
	{
	}
public:
	virtual ~Parser()
	...
	virtual inline std::string getFilename() const
	{
