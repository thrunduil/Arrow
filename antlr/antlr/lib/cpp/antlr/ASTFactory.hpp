...
typedef RefAST (*factory_type_)(antlr::pool_parser pool);
...
public:
	 ...
	 * instance)
	 */
	ASTFactory(pool_parser pool, const char* factory_node_name, factory_type factory );
	/// Destroy factory
	virtual ~ASTFactory();
	...
	RefAST create(const std::string& txt, ANTLR_USE_NAMESPACE(std)istream& infile );
...
protected:
	...
	inline RefAST getNodeOfType( unsigned int type )
	{
		return RefAST(nodeFactories[type]->second(pool));
	}
	/// get the name of the node 'type'
	const char* getASTNodeType( unsigned int type )
	{
