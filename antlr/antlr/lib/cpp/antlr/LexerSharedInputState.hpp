...
public:
	 ...
	 */
	LexerInputState(pool_parser pool,InputBuffer* inbuf)
	: column(1)
	, pool(pool)
	, line(1)
	, tokenStartColumn(1)
	 ...
	 */
	LexerInputState(pool_parser pool,ANTLR_USE_NAMESPACE(std)istream& in)
	: column(1)
	, pool(pool)
	, line(1)
	, tokenStartColumn(1)
	...
	, guessing(0)
	, filename("")
	, input(new(*pool,get_ptr<CharBuffer>()) CharBuffer(in))
	, inputResponsible(true)
	{
	}
