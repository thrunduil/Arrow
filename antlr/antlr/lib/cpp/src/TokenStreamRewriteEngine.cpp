...
#include <iostream>
#include <iterator>
//#include <antlr/stl/antlr_sstream.h>
#include <cassert>
...
const char* const TokenStreamRewriteEngine::DEFAULT_PROGRAM_NAME = "default";
...
TokenStreamRewriteEngine::TokenStreamRewriteEngine(pool_parser pool,TokenStream& upstream)
: TokenStream(pool)
, stream(upstream)
...
TokenStreamRewriteEngine::TokenStreamRewriteEngine(pool_parser pool,TokenStream& upstream, size_t initialSize )
: TokenStream(pool)
, stream(upstream)
, index(MIN_TOKEN_INDEX)
, tokens(initialSize)
...
//			std::cout << "tokenCursor = " << tokenCursor << " first prog index = " << (*rewriteOpIndex)->getIndex() << std::endl;
			...
			{
				size_t up_to_here = details::minimum(lastToken,(*rewriteOpIndex)->getIndex());
