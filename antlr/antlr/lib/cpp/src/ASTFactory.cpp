...
/// Initialize factory
ASTFactory::ASTFactory(pool_parser pool)
: pool(pool),default_factory_descriptor(ANTLR_USE_NAMESPACE(std)make_pair(CommonAST::TYPE_NAME,&CommonAST::factory))
{
 ...
 */
ASTFactory::ASTFactory(pool_parser pool, const char* factory_node_name, factory_type fact )
: pool(pool),default_factory_descriptor(ANTLR_USE_NAMESPACE(std)make_pair(factory_node_name, fact))
{
	nodeFactories.resize( Token::MIN_USER_TYPE, &default_factory_descriptor );
}
...
void ASTFactory::registerFactory( int type, const char* ast_name, factory_type factory )
{
	...
	// And add new thing..
	typedef ANTLR_USE_NAMESPACE(std)pair<const char*, factory_type> Type;
	nodeFactories[type] = new(*pool,get_ptr<Type>()) ANTLR_USE_NAMESPACE(std)pair<const char*, factory_type>( ast_name, factory );
}
...
{
	RefAST node = nodeFactories[0]->second(pool);
...
{
	RefAST t = nodeFactories[type]->second(pool);
	t->initialize(type,"");
	return t;
}
...
RefAST ASTFactory::create(const std::string& type_name, ANTLR_USE_NAMESPACE(std)istream& infile )
{
...
//	cout << "create(tr)" << endl;
	...
	RefAST t = nodeFactories[tr->getType()]->second(pool);
	t->initialize(tr);
	return t;
}
...
//	cout << "create( tok="<< tok->getType() << ", " << tok->getText() << ")" << nodeFactories.size() << endl;
	RefAST t = nodeFactories[tok->getType()]->second(pool);
	t->initialize(tok);
	return t;
}
