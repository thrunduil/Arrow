...
CharScanner::CharScanner(pool_parser pool,InputBuffer& cb, bool case_sensitive )
	: TokenStream(pool)
	, saveConsumedInput(true) //, caseSensitiveLiterals(true)
	, caseSensitive(case_sensitive)
	, literals(CharScannerLiteralsLess(this))
	, inputState(new(*pool,get_ptr<LexerInputState>()) LexerInputState(pool,cb))
	, commitToPath(false)
	, tabsize(8)
	, traceDepth(0)
{
...
CharScanner::CharScanner(pool_parser pool, const LexerSharedInputState& state, bool case_sensitive )
	: TokenStream(pool)
	, saveConsumedInput(true) //, caseSensitiveLiterals(true)
	, caseSensitive(case_sensitive)
	, literals(CharScannerLiteralsLess(this))
