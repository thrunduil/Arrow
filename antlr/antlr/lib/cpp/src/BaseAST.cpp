		...
		RefAST target,bool partialMatch)
{
	...
	for (RefAST sibling=RefAST(this);
			sibling;
 ...
 */
bool BaseAST::equalsList(RefAST t)
{
 ...
 */
bool BaseAST::equalsListPartial(RefAST sub)
{
 ...
 *  of 'this' are ignored.
 */
bool BaseAST::equalsTree(RefAST t)
{
	// check roots first
	if (!equals(t))
		return false;
 ...
 *  of 'this' are ignored.
 */
bool BaseAST::equalsTreePartial(RefAST sub)
{
	// the empty tree is always a subset of any tree.
	if (!sub)
		return true;
