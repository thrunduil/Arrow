...
TokenStreamSelector::TokenStreamSelector(pool_parser pool)
: TokenStream(pool),input(0)
{
}
...
void TokenStreamSelector::addInputStream(TokenStream* stream, const std::string& key)
{
...
TokenStream* TokenStreamSelector::getStream(const std::string& sname) const
{
	...
	if (i == inputStreamNames.end()) {
		throw std::string("TokenStream ")+sname+" not found";
	}
	return (*i).second;
}
